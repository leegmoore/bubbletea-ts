...
| D-057 | 2025-11-15 | Resize signal gating | SIGWINCH propagation now requires a TTY-backed renderer and honors the shared `ignoreSignals` flag (driven by `WithoutSignals`/`ReleaseTerminal`) before dispatching `WindowSizeMsg`, while still emitting the initial measurement eagerly. | Final |
| D-058 | 2025-11-15 | Suspend spec split | `suspend_unix_test.go` now captures renderer/ignoreSignals semantics for `Program.suspend()`, and the TypeScript suspend specs live in `packages/tests/src/program/suspend.test.ts` so runtime changes stay downstream of the Go oracle. | Final |
...
| D-063 | 2025-11-15 | Spinner test helper | Centralized the deterministic spinner fake in `packages/tests/src/integration/utils/fake-spinner.ts` so future specs reuse the same Tick semantics instead of re-implementing them. | Final |
